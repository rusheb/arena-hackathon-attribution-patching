...
# %%
filter_is_val = lambda name: True#lambda name: name.endswith("hook_v") or name.endswith("hook_z") or name.endswith("ln1.hook_scale") # or name.endswith("hook_v_input")
...
def get_full_vector_grad_input(grad_cache) -> Float[Tensor, "qkv layer batch pos head_index d_model"]:
    return t.stack([get_stacked_head_vector_grad_input(grad_cache, activation_name) for activation_name in [' ']], dim=0) # ['q', 'k', 'v']
